...
def _payload(result: Any) -> tuple[list[Mapping[str, Any]], Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    if status_code != 200:
        raise RuntimeError(
            f"Toobit Position endpoint returned HTTP {status_code}."
        )

    json_method = getattr(response, "json", None)
    if not callable(json_method):
        raise RuntimeError("Toobit Position response has no JSON parser")

    try:
        payload = json_method()
    except Exception as exc:
        raise RuntimeError(
            f"Toobit Position response JSON parsing failed: {exc}"
        ) from None
    ...
    if isinstance(payload, list):
        ...
        raise RuntimeError(
            "Toobit Position payload must contain a dynamic list"
        )
...
def _normalize_row(
    ...
    *,
    ...
    return {
        ...
        "source_id": source_id,
        "source_type": source_type,
        **({"source_timestamp": source_timestamp} if source_timestamp is not None else {}),
    }
...
def build_toobit_position_reader(adapter: Any):
    ...
    def read_positions() -> PositionReaderResult:
        ...
        return PositionReaderResult(
            ...
            status="PASS",
            reason="Authenticated Toobit Position read succeeded.",
        )
