...
def test_empty_balances_are_representable():
    observation = build_account_balance_observation(_adapter([]))
...
def test_invalid_account_fails_closed():
    ...
    try:
        build_account_balance_observation(adapter)
    except RuntimeError:
        return
...
def test_no_fixed_asset_count_assumption():
    ...
    observation = build_account_balance_observation(_adapter(rows))
